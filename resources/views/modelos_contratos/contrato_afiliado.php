...
<p style="text-align:justify">
    ...
    <strong style="text-transform:uppercase"><?=$afiliado->razaoSocial?></strong>, comercialmente conhecido como <strong style="text-transform:uppercase"><?=$afiliado->nomeFantasia?></strong>, localizada no endereço <span style="text-transform:capitalize"><?=$afiliado->endereco?> N°&nbsp;<?=$afiliado->numero?>&nbsp;<?php if(!is_null($afiliado->complemento)):?>-&nbsp;<?=trim($afiliado->complemento)?><?php endif;?>, <?=$afiliado->bairro?> - CEP: <?=formatarCEP($afiliado->cep)?> - <?=$afiliado->nomeCidade?>/<span style="text-transform: uppercase"><?=$afiliado->uf?></span></span>, inscrito no CNPJ sob o N° <strong><?=formatarCNPJ($afiliado->CNPJ)?></strong>.
    <br>
    Neste ato representado pelo(a) Sr(a). <strong style="text-transform:uppercase"><?=$afiliado->nomeResponsavel?></strong>, inscrito no CPF N° <strong><?=formatarCPF($afiliado->CPF)?></strong>, denominado simplesmente <strong>CONTRATANTE</strong>.
    <br><br>
    <strong>CONTRATADO: GILSON RIDHER RATIER QUEIROZ - ME</strong>, comercialmente conhecido como <strong>CASA DO SÍNDICO</strong>, localizado no endereço Rua Wanderley Júnior N° 5 - Sala 18, Campinas - Cep: 88101-010 - São José/SC, inscrito no CNPJ sob o N° <strong>22.912.131/0001-88</strong>.
    <br>
    ...
    <br>
    O presente contrato inicia-se em <?=formatarDataExtenso($afiliado->dataContrato)?>, tendo vigência de no mínimo três meses após esse período por prazo indeterminado.
    <br><br>

    <strong>Cláusula Terceira – DO VALOR</strong>
    <br>
    <?php if($afiliado->valorComissao == 0):?>
    O valor contratual será de <?=formatarValor($afiliado->valorPlano)?> (<?=$afiliado->valorPlanoExtenso?>) mensais, para empresas de manutenção predial e serviços técnicos.
    <?php else:?>
    O valor contratual será de <?=formatarValor($afiliado->valorPlano)?> (<?=$afiliado->valorPlanoExtenso?>) mensais mais <?=formatarPercentual($afiliado->valorComissao)?> da mão de obra referente aos serviços indicados pela CASA DO SÍNDICO.
    <?php endif;?>
    <br><br>
