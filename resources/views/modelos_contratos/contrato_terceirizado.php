<div style="width:100%">
    <img src="http://<?=$_SERVER['SERVER_NAME']?>/assets/images/casa_logo.png" />
    <br>
    <h2>Contrato de Prestação de Serviços</h2>
</div>
<p style="text-align:justify">
    Contrato de prestação de serviços que entre si fazem a empresa <strong style="text-transform:uppercase"><?=$afiliado->razaoSocial?></strong>, comercialmente conhecido como <strong style="text-transform:uppercase"><?=$afiliado->nomeFantasia?></strong>, sitio a Rua <span style="text-transform:capitalize"><?=$afiliado->endereco?> N°&nbsp;<?=$afiliado->numero?>&nbsp;<?php if(!is_null($afiliado->complemento)):?>-&nbsp;<?=trim($afiliado->complemento)?><?php endif;?>, <?=$afiliado->bairro?> - CEP: <?=formatarCEP($afiliado->cep)?> - <?=$afiliado->nomeCidade?>/<span style="text-transform: uppercase"><?=$afiliado->uf?></span></span>, inscrito no CNPJ sob o N° <strong><?=formatarCNPJ($afiliado->CNPJ)?></strong>.
    <br>
    Neste ato representado peloseu administrador, inscrito no CPF N° <strong><?=formatarCPF($afiliado->CPF)?></strong>, denominado simplesmente <strong>CONTRATANTE</strong>,
    e a <strong>CASA DO SINDICO</strong>, pessoa jurídica de direito privado, inscrita no CNPJ sob o N° <strong>22.912.131/0001-88</strong>, sediada na Rua General Liberato, nº 1914, sala 104 - Canto - Florianópolis/SC - CEP 88.101-010, doravente denominada simplesmente <strong>CONTRATADA</strong>, de acordo com as cláusulas e condições que segue:
    <br><br>
    ...
    <br>
    O presente contrato inicia-se em <?=formatarDataExtenso($afiliado->dataContrato)?>, tendo vigência de três meses após esse período o contrato será por prazo indeterminado, podendo ser rescindido a qualquer momento por qualquer das partes, sem motivo procedente, com aviso prévio de 30 dias sem que haja multas para ambos os lados desde que não houve prejuízo predecessores.
    <br>
    ...
    <strong>Cláusula Terceira – DO VALOR E FORMA DE PAGAMENGO</strong>
    <br>
    1º - O valor contratual é de <?=formatarValor($afiliado->valorPlano)?> (<?=$afiliado->valorPlanoExtenso?>) mensais, até que seja concretizada a primeira venda, após essa concretização a <strong>CONTRATANTE</strong>, obriga-se a pagar a mensalidade mais o valor proporcional de <?=formatarPercentual($afiliado->valorComissao)?> da soma total de cada contrato.
    <br>
    ...
    4° - Fica eleito o FORO DA COMARCA DE SÃO JOSÉ/SC, como o competente para dirimir as questões decorrentes da execução deste contrato, renunciando outro mais privilegiado que sejam como também os casos omissos e não regulado pelo presente instrumento.<br>
    <br>

    E, por estarem justos e contratados assinam o presente instrumento juntamente com as testemunhas abaixo assinadas.
    <br><br>
    São José, <?=formatarDataExtenso($afiliado->dataContrato)?>.
    <br><br><br>
