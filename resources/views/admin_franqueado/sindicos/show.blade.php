      ...
      <div class="col-md-6">
        <div class="card">
          <div class="card-body">
            ...
            @endif
            <h4 style="margin-top:0px;" id="nome-condominio">
  @if($sindico->usuarioApp && isset($sindico->usuarioApp->email))
    <a href="mailto:{{ $sindico->usuarioApp->email }}" target="_blank">{{ $sindico->usuarioApp->email }}</a>
  @else
            ...
            <table id="table_condominios" data-page-length="10" id="dataTableExample" class="table table-striped dataTable no-footer" role="grid" aria-describedby="dataTableExample_info">
              ...
              <tbody>
                ...
                if ($faixaCep) {
                  ...
                  if ($regiaoFranquiado->franqueado->id == $franqueado_id) {
                    ...
                    <tr>
                      ...
                      <td>
                        ...
                        if ($faixaCep) {
                          ...
                          $regiao_name = "<span class='badge badge-secondary'>Região " . ($regiaoFranquiado->regiao ? $regiaoFranquiado->regiao->nome : "Sem região") . "</span>";
                        } else {
                      ...
                      <td style="text-align: center;">
                        ...
                        @endif
                        <a style="font-size: 12px;" href="javascript:void(0)" onclick="carregarSolicitacoes(<?php echo $condominio->id; ?>, '<?php echo $condominio->nome; ?>')" class="btn btn-primary"><i data-feather="eye" style="width: 20px;"></i> Ver ({{$countTotal}})</a>
                        @if($condominio->status === 'ativo')
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-body">
            ...
            <div class="container_solicitacoes row"></div>
          </div>
        </div>
      </div>
    </div>
...
<div class="pull-right" style="margin-top: 40px;">
  <form method="POST" action="{!! route('admin_franqueado.sindicos.destroy', $sindico->id) !!}" accept-charset="UTF-8">
    <input name="_method" value="DELETE" type="hidden">
    {{ csrf_field() }}
    <div class="btn-group btn-group-sm" role="group">

      <a href="{{ route('admin_franqueado.sindicos.index') }}" class="btn btn-primary" title="Ver todos sindicos">
        <i data-feather="list"></i>
      </a>

      <a href="{{ route('admin_franqueado.sindicos.create') }}" class="btn btn-success" title="Novo sindico">
        <i data-feather="plus"></i>
      </a>

      <a href="{{ route('admin_franqueado.sindicos.edit', $sindico->id ) }}" class="btn btn-primary" title="Editar sindico">
