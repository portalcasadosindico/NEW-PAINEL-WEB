    ...
    <div class="col-md-6">
        ...
        <div class="card" id="changestatus">
            <div class="card-body">
                ...
                <div class="form-group">
                    <label for="status">Status Oficial para esta solicitação ou serviço</label>
                    <?php echo StatusOrcamento::getSelectAllStatus('status', 'status', 'Selecione um item', $orcamento->status, "showDescricaoCancelamento(this.value)"); ?>
                    @error('status')
                    <label id="status-error" class="error mt-2 text-danger" for="status">{{ $message }}</label>
                    @enderror
                </div>

                <div class="form-group motivo-cancelamento" style="display: none;">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div>
                            ...
                            <label class="mail-link">{{$orcamento->condominio()->withTrashed()->first()->sindico()->withTrashed()->first()->usuarioApp()->withTrashed()->first()->email}}</label>
                            <div>
        ...
        <div class="card" id="add_afiliado">
            <div class="card-body">
                ...
                <div class="">
                    ...
                    @if($orcamento->afiliado()->withTrashed()->first()==null)
                                            ...
                                            <tbody>
                                                ...
                                                <tr role="row">
                                                    <td>
                                                        ...
                                                        <span style="display:none;"><?php echo str_replace('-', '', Formatacao::chave($afiliado->razao_social . '' . $afiliado->nome_fantasia . '' . $afiliado->telefone)); ?></span>
                                                    </td>
                                                    <td>{{ $afiliado->telefone }}</td>
                                                    <td>
                                                        @foreach($afiliado->categorias as $afiliadoCategoria)
                                                        @if($afiliadoCategoria->categoria &&
                                                        $afiliadoCategoria->categoria->status=="aprovado")
                                                        <p class="badge badge-success">
                                                            {{$afiliadoCategoria->categoria->nome}}
                                                        </p>
                                                        @endif
                                                        @endforeach
                                                    </td>
                                                    ...
                                                    <td><a href="javascript:void(0)" data-dismiss="modal" class="btn btn-success" onclick="selecionarAfiliado('{{ $afiliado->id }}','{{ $afiliado->nome_fantasia ? rawurlencode($afiliado->nome_fantasia) : rawurlencode($afiliado->razao_social) }}','{{ $afiliado->telefone }}')">Selecionar</a>
                                                    </td>
                                                </tr>
                                                @endforeach
                                            </tbody>
                                                ...
                                                <tr role="row">
                                                    ...
                                                    <td>
                                                        ...
                                                        <label class="badge badge-success">
                                                            {{$afiliadoCategoria->categoria->nome}}</label>
                                                        @endif
                                                        @endforeach
                                                    </td>
                                                    <td>
                ...
                <div class="mt-3">
                    ...
                    <label class="badge badge-primary">Contrato visualizado em
                        <?php echo Formatacao::data($assinaturas["afiliado"]["assinatura"]->viewed, false, false); ?></label>
                    @else
                    ...
                    <label class="badge badge-success" title="Autenticado pelo Autentique">Contrato Assinado em
                        <?php echo Formatacao::data($assinaturas["afiliado"]["assinatura"]->signed, false, false); ?></label>
                    @endif
        ...
        <div class="card">
            <div class="card-body">
                ...
                <select onchange="showAssinaturas(this.value)" class="hide-assinatura" name="formato_contrato_atual" style="margin-bottom: 24px; font-weight: bold;">
                    <option <?php if ($orcamento->formato_contrato_atual == 4) echo "selected"; ?> value="4">Vou enviar o
                        contrato depois</option>
                    <option <?php if ($orcamento->formato_contrato_atual == 1) echo "selected"; ?> value="1">Este contrato
                        ainda não foi assinado por ninguém</option>
                    <option <?php if ($orcamento->formato_contrato_atual == 2) echo "selected"; ?> value="2">Este contrato
                        já foi assinado por todos</option>
                    <!--<option <?php // if($orcamento->formato_contrato_atual==3) echo "selected"; 
                                ?> value="3">Este contrato foi parcialmente assinado</option>-->
                </select>
                @else
                ...
                <!--<div style="width: 100%; max-width: 300px;">
                    <div class="form-group assinatura assinatura-sindico">
                        <label class="display-block">Data assinatura síndico</label>
                        <label class="badge badge-success">dd/mm/yyyy</label>
                    </div>

                    <div class="form-group assinatura assinatura-sindico">
                        <label class="display-block">Data assinatura afiliado</label>
                        <label class="badge badge-success">dd/mm/yyyy</label>
                    </div>

                    <div class="form-group assinatura assinatura-sindico">
                        <label class="display-block">Data assinatura franqueado</label>
                        <label class="badge badge-success">dd/mm/yyyy</label>
                    </div>
                </div>  -->
...
<button type="submit" class="btn btn-primary mr-2 hide-btn-send">Salvar</button>
<a href="{{route('admin.orcamentos.index')}}" class="btn btn-light">Cancelar</a>
