...
class Asaas{
        ...
        public static function novoCustomer($afiliado, $token, $updateAfiliado=true, $franqueado_id=null){
            ...
            $response = Http::withHeaders([
                'access_token' => $token,
            ])->post(self::getUrlAsaas()."/customers",(array) $customer);
            ...
            //}
            //#####################################################################################################################################################
        // Alterei aqui, verificação de ID na response da API do ASSAS
        ...
        if ($response->successful()) {
            ...
            if (isset($responseData['id'])) {
                if($updateAfiliado) {
                    ...
                    $afiliadoAsaas->afiliado_id = $afiliado->id;
                    $afiliadoAsaas->franqueado_id = $franqueado_id;
                    $afiliadoAsaas->asaas_customer_id = $responseData['id'];
                    $afiliadoAsaas->modo = Util::getModusOperandi();
                    $afiliadoAsaas->save();
                }
            } else {
          
                dd("ID não encontrado na resposta", $responseData);
            }
        } else {
           
            dd("Erro na requisição: Status", $response->status(), $response->json());
        ...
        public static function getCobrancas($customer_id, $token){
            $response = Http::withHeaders([
                'access_token' => $token,
            ])->get(self::getUrlAsaas()."/payments?limit=100&customer=".$customer_id);
            return $response->json();
        }
