...
class Validacao
{
    ...
    public function email($vcampo, $vdado, $campo_message)
    {
        ...
        if ($vdado != "" && !is_null($vdado) && !filter_var($vdado, FILTER_VALIDATE_EMAIL)) {
            $this->mensagem[] = ["error_code" => "invalid-" . $vcampo, "error_message" => $campo_message . " inválido"];
        }
    }
    ...
    public function inteiro($vcampo, $vdado, $campo_message)
    {
        ...
        if (!is_null($vdado) && !filter_var($vdado, FILTER_VALIDATE_INT)) {
            $this->mensagem[] = ["error_code" => "invalid-" . $vcampo, "error_message" => $campo_message . " aceita apenas dados inteiros (1; 2; 3; etc...)"];
        }
    }
    ...
    public function real($vcampo, $vdado, $campo_message)
    {
        ...
        if (!is_null($vdado) && !filter_var($vdado, FILTER_VALIDATE_FLOAT)) {
            $this->mensagem[] = ["error_code" => "invalid-" . $vcampo, "error_message" => $campo_message . " aceita apenas dados numéricos reais (1,23; 1,10; 2; 5; etc...)"];
        }
    }
    ...
    {
        if ($vdado == "") {
            $this->mensagem[] = ["error_code" => "required-" . $vcampo, "error_message" => $campo_message . " é obrigatório"];
        }
    }
    ...
    public function validarCpfGeral($vcampo, $vdado, $campo_message)
    {
        if ($this->validarCPF($vdado) == false) {
            $this->mensagem[] = ["error_code" => "invalid-" . $vcampo, "error_message" => $campo_message . " é inválido"];
        }
    }
    ...
    public function tamanho_string($vcampo, $vdado, $min, $max, $campo_message)
    {
        if ($vdado != "" && !is_null($vdado) && !(strlen($vdado) >= $min && strlen($vdado) <= $max)) {
            $this->mensagem[] = ["error_code" => "invalid-" . $vcampo, "error_message" => $campo_message . " deve ter entre " . $min . " e " . $max . " caracteres"];
        }
    }
    ...
    {
        return ["error_code" => "invalid-" . $code, "error_message" => $messge];
    }
}
